...
// NO BORRAR LO DE ARRIBA HASTA ESTAR SEGURO QUE LO DE ABAJO FUNCIONA BIEN  V.2 //
...
//       // Primero aseguramos que los totales estén bien calculados
//       calcularTotales();

//       // Luego normalizamos TODOS los campos numéricos a "1234.56"
//       camposNumericos.forEach(function (el) {
//         if (!el) return;
//         var n = convertirAFloat(el.value);
//         el.value = n ? n.toFixed(2) : "0.00";
...
//       // ⚠️ NO volvemos a llamar a calcularTotales() acá,
//       // para no re-formatear a "12.345,67" justo antes del submit.
//     });
//   }
// });

// NO BORRAR LO DE ARRIBA HASTA ESTAR SEGURO QUE LO DE ABAJO FUNCIONA BIEN  V.3 //
...
// // === DOMContentLoaded ===
// document.addEventListener("DOMContentLoaded", function () {
//   // 🔹 Detectar si estamos en la vista de EDITAR viático
//   var esVistaEditarViatico =
//     window.location.pathname.indexOf("/viaticos/") !== -1 &&
//     window.location.pathname.indexOf("editar") !== -1;
...
//   // 🚫 Al inicio: TODOS los importes bloqueados hasta que haya viajante válido
//   //    PERO solo en la vista de CREAR. En EDITAR los queremos editables.
//   if (!esVistaEditarViatico) {
//     document.querySelectorAll(".importe").forEach(function (inp) {
//       inp.readOnly = true;
//       inp.classList.add("bg-gray-100");
//     });
...
// NUEVA VERSION VERIFICAR FUNCIONAMIENTO Y BORRAR LO ANTERIO //
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // En CREAR: importes bloqueados hasta elegir viajante
  if (!esVistaEditarViatico) {
    document.querySelectorAll(".importe").forEach(function (inp) {
      inp.readOnly = true;
      inp.classList.add("bg-gray-100");
  ...
  // Sincronizar cargo y evitar duplicados
  var selectsViajantes = document.querySelectorAll(".viajante-select");
  selectsViajantes.forEach(function (sel) {
    sel.dataset.prevValue = sel.value || "";
  });
  ...
  if (checkVales && contVales) {
    checkVales.addEventListener("change", toggleVales);
    // En editar puede venir ya checkeado
    toggleVales();
  }
  ...
  // === Validación ANTES de enviar (CREAR y EDITAR) ===
  var form = document.getElementById("formularioViatico");
  if (form) {
    form.addEventListener("submit", function (e) {
      // 🔹 Limpiar marcas previas de error
      document.querySelectorAll(".fila-viajante-error").forEach(function (tr) {
        tr.classList.remove(
          "fila-viajante-error",
          "ring-2",
          "ring-red-400",
        ...
        // Restaurar fondo por defecto (odd/even de Tailwind)
        tr.style.backgroundColor = "";
      });

      document.querySelectorAll(".viajante-select").forEach(function (sel) {
        sel.classList.remove("border-red-500", "bg-red-50");
      });

      // 1) Validar que todas las filas activas tengan viajante
      if (cantViajantesSel) {
        var cant = parseInt(cantViajantesSel.value || "0", 10);
        if (isNaN(cant) || cant < 0) cant = 0;
        ...
        var selects = document.querySelectorAll(".viajante-select");
        var filasConError = [];
        var primeraFilaConErrorSelect = null;

        for (var i = 0; i < cant; i++) {
          var sel = selects[i];
          if (!sel || !sel.value) {
            filasConError.push(i + 1); // nro visible

            // 🔴 Marcar visualmente la fila de error
            if (sel) {
              ...
              if (tr) {
                ...
                // Forzar fondo rojo también en filas even (override even:bg-*)
                tr.style.backgroundColor = "#fee2e2"; // similar a bg-red-100
              }
              sel.classList.add("border-red-500", "bg-red-50");

              if (!primeraFilaConErrorSelect) {
                primeraFilaConErrorSelect = sel;
              }
            }
          }
        }
        ...
        if (filasConError.length > 0) {
          ...
          if (filasConError.length === 1) {
            // Solo una fila con error
            mensaje =
              `En la fila ${filasConError[0]} no hay viajante seleccionado.\n\n` +
              `Corregí la selección o disminuí la cantidad de viajantes.`;
          } else {
            // Varias filas con error
            mensaje =
              `En las filas ${filasConError.join(", ")} no hay viajante seleccionado.\n\n` +
              `Corregí la selección o disminuí la cantidad de viajantes.`;
          }

          alert(mensaje);

          // Llevar el foco a la primera fila con error
          if (primeraFilaConErrorSelect) {
            primeraFilaConErrorSelect.focus();
            primeraFilaConErrorSelect.scrollIntoView({
              behavior: "smooth",
              block: "center",
            });
          }
      ...
      // 2) Calcular totales y normalizar números para el backend
      var camposNumericos = [];
      document.querySelectorAll(".importe").forEach(function (el) {
        camposNumericos.push(el);
      });
      ...
      // Aseguramos que totales estén bien
      calcularTotales();

      // Normalizamos a "1234.56"
      camposNumericos.forEach(function (el) {
        if (!el) return;
        var n = convertirAFloat(el.value);
        el.value = n ? n.toFixed(2) : "0.00";
